...
Memento* PoolGame::saveToMemento(){
    m_states.clear();

    for(Ball* b:m_balls){
        m_states.push_back(b->clone());
    }
    return new Memento(m_states);
}

void PoolGame::restoreFromMemento(Memento memento){
    m_balls.clear();
    for(Ball* b: memento.getSavedState()){
        m_balls.push_back(b);
    }
}
