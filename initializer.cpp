...
PoolGame *Initializer::createPoolgame(const std::string &configFilePath, Dialog *parent)
{
    ...
    if(config.contains("ballGroup"))
    {
        QJsonArray balls2 = config["ballGroup"].toArray();
        for(int i = 0; i < balls2.size();++i)
        {

            builder.addBallGroup(balls2[i].toObject(),config["table"].toObject());
        }
    }
    else
    {
        std::cout << "no \"ballGroup\" key found" <<std::endl;
        return nullptr;
      }
    }
